...
- GitHub Releases의 `Zotero-Read-Only-Assistant.exe`
- Zotero Desktop 다운로드 링크
...
3. `Zotero-Read-Only-Assistant.exe`를 더블클릭합니다.
4. 창에서 `검색 화면 열기` 버튼을 누릅니다.
5. 브라우저에 `Zotero 읽기 도우미`가 열리면 검색합니다.
6. 다음부터는 Zotero를 먼저 열고 같은 실행 파일을 더블클릭하면 됩니다.
...
- 참가자에게 Python을 설치하도록 안내할 필요가 없습니다.
